...
class eqdsk:
    ...
    def get_flux_contour(self):
        ...
        psif = RegularGridInterpolator((self.R,self.Z),self.psirz.T)
        points = np.vstack((RR.ravel(), ZZ.ravel())).T
        self.PSIRZ = psif(points).reshape(self.nh, self.nw)
        return
    ...
    def get_ext_bnd(self):
        ...
        self.rzbdyt = np.zeros((ndat,2))
        for i in range(ndat):
            line = f.readline().split()
            self.rzbdyt[i,0] = float(line[0])
            self.rzbdyt[i,1] = float(line[1])
        f.close()
        return
    ...
    def make_rho_R_psin(self):
        ...
        psif = RegularGridInterpolator((self.R,self.Z),self.psirz.T, method='linear')

        zmag_array = np.full_like(RR, self.zmag)
        points = np.vstack((RR, zmag_array)).T
        psir   = psif(points)

        zmag_array2 = np.full_like(RR2, self.zmag)
        points2= np.vstack((RR2, zmag_array2)).T
        psir2  = psif(points2)                 
    ...
    def construct_volume(self):
        ...
        self.psif = RegularGridInterpolator((self.R,self.Z),(self.psirz-self.smag).T/(self.sbdy-self.smag))
        ...
        for i in range(len1):
            ...
            points = np.vstack([rr, zz]).T 
            psi    = self.psif(points)
        ...
        for i in range(len2-1):
            for j in range(len1):
                ...
                if self.isfield:
                    point = np.array([[Rc, Zc]]) 
                    bn = self.bnf(point)
                    sum3 = sum3 + 2.*np.pi*Rc*dA*bn
    ...
    def make_chease_expeq(self,filename='EXPEQ'):
        ...
        if not (self.zeroedgep):
            f4.write('%f \n'%(self.pscale*mu0 * self.pres[-1] / (self.B0EXP **2)))
        else:
            f4.write('0.00 \n')
        f4.write('%i \n'%len(rz))
...
#       chease_namelist1.append('                               CWIDTH=0.1, .10,.02,.05,\n');
    ...
    def shape_postprocess(self,simple=False):
        ...
        target_psin_old = self.target_psin
        self.target_psin = 0.9993
        self.get_target_bnd()
        self.target_psin = target_psin_old
        ...
        for i in range(len1-1):
            ...
            perim = perim + dL;
            surf  = surf  + 2.* np.pi * dL * rc

        perim = perim * 1.005 #compensate xpoint
        big_ind = 1
        small_ind = 1
        ...
        for i in range(len1-1):
            ...
            if (self.rzbdyt[i,1] > self.rzbdyt[big_ind,1]):
                big_ind = i

        for i in range(len1-1):

            if (self.rzbdyt[i,1] < self.rzbdyt[small_ind,1]):
                small_ind = i

        a = (max(self.rzbdyt[:,0]) - min(self.rzbdyt[:,0])) * 0.5
        R = (max(self.rzbdyt[:,0]) + min(self.rzbdyt[:,0])) * 0.5

        r_up = self.rzbdyt[big_ind,0]
        r_low = self.rzbdyt[small_ind,0]
            
        self.elong2 = (self.rzbdyt[big_ind,1]-self.rzbdyt[small_ind,1])/a * 0.5
        self.triu = (R-r_up)/a
        self.tril = (R-r_low)/a
        self.perim = perim
        self.surf  = surf
        ...
        bcentr = abs(self.fpol[-1]/R)

        self.bp   = 4 * self.pva * self.vol / (self.rcentr * mu0 * self.ip**2)
        self.bp2  = self.pva * 2. * mu0 / (self.bpavg**2)
        self.beta = self.pva * 2. * mu0 / (bcentr**2)
        self.betan= self.beta* a * bcentr / abs(self.ip) * 1.e8
        self.li   = 2 * self.bpa * self.vol / (self.rcentr * mu0**2 * self.ip**2)
        return
...
#       txt.set_clip_on(False)
    ...
    def read_namelist(self,filename):
        ...
        while True:
            ...
            self.pscale= self.read_namelist_str(line,'PSCALE',self.pscale,2)    
        return
    ...
    def read_eqdsk_file(self,simple=False):
        ...
        self.isfield = False
        if self.isfield: self.field_postprocess()
        self.construct_volume()
        if self.isfield: self.get_xpoint()
        self.shape_postprocess()
        self.get_target_bnd()
        return

    def field_postprocess(self):
        if not self.isfield: return
        otime = time.time()
        self.construct_2d_field()   
        self.bnf  = RegularGridInterpolator((self.r,self.z),(self.br**2+self.bz**2).T)
        print('>>> Elapsed time [s]',time.time()-otime)
        return
    ...
    def get_xpoint(self):
        ...
        zmax = np.max(self.rzbdy[:,1]); zmaxloc = np.argmax(self.rzbdy[:,1]); rmax = self.rzbdy[zmaxloc,0]
        zmin = np.min(self.rzbdy[:,1]); zminloc = np.argmin(self.rzbdy[:,1]); rmin = self.rzbdy[zminloc,0]

        #find upper-xpnt
        epx = 1.e0; epz = 1.e0; xx = rmax; zz= zmax; count1 = 0; count2 = 0;
        while ((epx > eps or epz > eps) and count1 < 100):
            zz2 = np.linspace(zmax,0.95*self.z[-1],int((0.95*self.z[-1]-zmax)/dz));
            points = np.vstack((np.full_like(zz2, xx), zz2)).T
            bnt = self.bnf(points)
            minb= np.min(bnt); minbloc = np.argmin(bnt)
            epz = (zz2[minbloc]-zz)/zz; zz = zz2[minbloc];
    
            xx2 = np.linspace(1.05*self.r[0],0.95*self.r[-1],int((0.95*self.r[-1]-1.05*self.r[0])/dr));
            points2 = np.vstack((xx2, np.full_like(xx2, zz))).T
            bnt = self.bnf(points2);
            minb= np.min(bnt); minbloc = np.argmin(bnt);
            epx = (xx2[minbloc]-xx)/xx; xx = xx2[minbloc];
            count1 += 1

        self.xpnt_R[0] = xx; self.xpnt_Z[0] = zz; self.xpnt_P[0] = self.psif([self.xpnt_R[0],self.xpnt_Z[0]])

        #find lower-xpnt
        epx = 1.e0; epz = 1.e0; xx = rmin; zz= zmin;
        while ((epx > eps or epz > eps) and count2 < 100):

            zz2 = np.linspace(1.05*self.z[0],zmin,int((zmin-1.05*self.z[0])/dz));
            points3 = np.vstack((np.full_like(zz2, xx), zz2)).T
            bnt = self.bnf(points3);
            minb= np.min(bnt); minbloc = np.argmin(bnt)
            epz = (zz2[minbloc]-zz)/zz; zz = zz2[minbloc];

            xx2 = np.linspace(1.05*self.r[0],0.95*self.r[-1],int((0.95*self.r[-1]-1.05*self.r[0])/dr));
            points4 = np.vstack((xx2, np.full_like(xx2, zz))).T
            bnt = self.bnf(points4);
            minb= np.min(bnt); minbloc = np.argmin(bnt);
            epx = (xx2[minbloc]-xx)/xx; xx = xx2[minbloc];
            count2 += 1
        ...
        self.xpnt_R[1] = xx; self.xpnt_Z[1] = zz; self.xpnt_P[1] = self.psif(self.xpnt_R[1],self.xpnt_Z[1])
        self.xpnt_P    = (self.xpnt_P - self.smag) / (self.sbdy - self.smag)
    ...
    def run_chease(self,save_gfile=False):
        ...
        f.close()
        bcentr = self.fpol[-1]/self.rcentr
