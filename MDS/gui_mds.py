...
class kstar_diagnostic_tool:
    ...
    def _load_ts(self):
        ...
        print('>>> Load TS CORE...')
        nmax = 30; nstart = 1;
        if self.year in ts_location_core: 
            nmax = len(ts_location_core[self.year])+1
        ...
        for flag in ['te','ne']:
            if   flag == 'te': 
                self.ts['core']['rr'] = np.array([])

            for ch in range(1,nmax):

                point_ch   = ch - 1 + nstart

                nfile      = self.savfile+'%s%i.npz'%(flag.upper(),ch)
                node_name1 = '\\TS_CORE%i.CORE%i_%s'%(point_ch,point_ch,flag.upper())
                node_name2 = '\\TS_CORE%i.CORE%i_%sRRH'%(point_ch,point_ch,flag.upper())
                node_name3 = '\\TS_CORE%i.CORE%i_POS'%(point_ch,point_ch)
                ...
                if not os.path.isfile(nfile+'.gz'):
                    ...
                    if time.shape[0]==0: 
                        break           
                    if err.shape[0]==0: 
                        err = val/10. #10% errorbar

                    if (len(time.shape)>1 and time.shape[0]==1):
                        time = time.T[:,0]
                        val  = val.T[:,0]
                        err  = err.T[:,0]

                    if self.year in ts_location_core:
                        rr=ts_location_core[self.year][ch-1]

                    self._get_save_zipf(nfile,[time,val,err,rr])
        ...
        print('>>> Load TS EDGE...')
        nmax = 30; nstart = 1;
        if self.year in ts_location_edge: 
            nmax = len(ts_location_edge[self.year])+1

        #Exception for 2024
        if (self.year==2024 and self.shotn>=35534):
            nstart = 2; nmax = 16;

        for flag in ['te','ne']:            
            if flag == 'te': 
                self.ts['edge']['rr'] = np.array([])    
            ...
            for ch in range(1,nmax):
                ...
                node_name1 = '\\TS_EDGE%i.EDGE%i_%s'%(point_ch,point_ch,flag.upper())
                node_name2 = '\\TS_EDGE%i.EDGE%i_%sRRH'%(point_ch,point_ch,flag.upper())
                node_name3 = '\\TS_EDGE%i.EDGE%i_POS'%(point_ch,point_ch)
                ...
                if not os.path.isfile(nfile+'.gz'):
                    ...
                    time = self.ts[flag]['edge'][ch][0]
                    val  = self.ts[flag]['edge'][ch][1]
                    err  = self.ts[flag]['edge_err'][ch][1]
                    rr   = rr[1][0];
                    ...
                    if self.year in ts_location_edge:
                        rr=ts_location_edge[self.year][point_ch-1]
            ...
            if self.ts['edge']['nch'] == 0: 
                print('>>> TS data unavail.');
            else: 
                print('>>> TS %s CORE/EDGE CH. %i/%i [#] '%(flag.upper(),self.ts['core']['nch'],self.ts['edge']['nch']))        
    ...
    def _load_eq(self):
        ...
        for efit_no in self.efit_list['isefit'].keys():
            ...
            self.gkfiles['eq'][efit_no].psif = RegularGridInterpolator((eq.R,eq.Z),(eq.psirz-eq.smag).T/(eq.sbdy-eq.smag))
            rho_map   = eq.prhoR[:,1]
            psi_map   = eq.prhoR[:,0]
    ...
    def _get_xmap(self):
        ...
        if self.ts['core']['nch']>3:
            ...
            z_array = np.full_like(self.rr['ts']['core'][1],0)
            point = np.array([self.rr['ts']['core'][1],z_array]).T
            self.rr['ts']['core'][2] = self.gkfiles['eq'][self.efit_no_g.get()].psif(point)
            self.rr['ts']['core'][3] = self.gkfiles['eq'][self.efit_no_g.get()].psi_to_rho(self.rr['ts']['core'][2])
    
            self.rr['ts']['edge'][1] = self.ts['edge']['rr']
            z_array = np.full_like(self.rr['ts']['edge'][1],0)
            point = np.array([self.rr['ts']['edge'][1],z_array]).T
            self.rr['ts']['edge'][2] = self.gkfiles['eq'][self.efit_no_g.get()].psif(point)
            self.rr['ts']['edge'][3] = self.gkfiles['eq'][self.efit_no_g.get()].psi_to_rho(self.rr['ts']['edge'][2])
        if self.ces['nch']>3:
            self.rr['ces'][1] = self.ces['rr']
            z_array = np.full_like(self.rr['ces'][1],0)
...
#       self.note_in['r8'].pack(side='left')
    ...
    def _draw_time_ts(self,time,xx1,nch1,xx2,nch2,data):
        ...
        xx = []; yy = []; yerr= [];
        for i in range(1,nch1+1):
            ind = np.argmin(abs(data['core'][i][0]-time/1.e3))
            xx.append(xx1[i-1])
            yy.append(data['core'][i][1][ind]);
            yerr.append(data['core_err'][i][1][ind]);
        for i in range(1,nch2+1):
            ind = np.argmin(abs(data['edge'][i][0]-time/1.e3))
            xx.append(xx2[i-1])
            yy.append(data['edge'][i][1][ind]);
            yerr.append(data['edge_err'][i][1][ind]);           
        np.nan_to_num(yy, copy=False); np.nan_to_num(yerr, copy=False)
