...
class fit_tool:
    ...
    def pretty_print(self,lm,err,colwidth=8, precision=4, fmt='g',columns=['value', 'min', 'max', 'stderr', 'vary', 'expr','brute_step']):
        ...
        for name, values in sorted(lm.items()):
            ...
            #Modify stderr part
            #Add minimum std if vary is True
            if pvalues['vary']: 
                err[count] = max(err[count],1.e-5)
    ...
    def read_raw_kinprof_fun(self,filename,shift,use_rho):
        ...
        while (i < linec):
            ...
            if (float(line[0]) > 0.):
                ...
                point = np.array([[datR[i],datZ[i]]])
                
                if (use_rho):
                    if (self.fit_eq['psif'](point) < 1.0):
                        datP[i] = self.fit_eq['psi_to_rho'](self.fit_eq['psif'](point))
                    else:
                        datP[i] = 1.0 + drdp*(self.fit_eq['psif'](point)-1.)
                else:
                    datP[i] = self.fit_eq['psif'](point)
                i = i + 1
    ...
    def get_tci_line(self,Rmid,Rend,nef,LL,use_rho):
        ...
        z_array = np.full_like(R2,0.);
        points  = np.vstack((R2,z_array)).T
        psi_den = self.fit_eq['psif'](points)
...
#               self.lmfit_set_param(self.param[flag],3,None, None,None,True)
    ...
    def lmfit_fit(self,flag):
        ...
        if (use_raw and not_use_avg_all):
            for i in range(oli_cutn):
                result = lmfit.minimize(self.residual,p,args=(datx,daty,dats,ch_list,func,flag),method='leastsq')
        ...
        try:   cov = result.covar
        except:cov = None
        self.post['poptc'][flag]=cov
    ...
    def eped_prof(self, x, a1=0, a2=0, a3=0, a4=0, a5=0, a6=0, a7=0, a8=0):
        ...
        cf = 0.5*(1.+np.tanh((a5-0.2)/1.e-2))*a5+0.1
        yt = (x/(1.0-a3)) ** cf
        cf = 0.5*(1.+np.tanh((a6-0.2)/1.e-2))*a6+0.1
        npstep = 1.+np.tanh((1.0-a3-x)/1.e-6)
        y = y + a4 * ((abs(1-yt)) ** cf) * 0.5 * npstep #(abs)
    ...
    def eped_prof2(self, x, a1=0, a2=0, a3=0, a4=0, a5=0, a6=0, a7=0, a8=0):
        ...
        cf = 0.5*(1.+np.tanh((a5-0.2)/1.e-2))*a5+0.1
        yt= (x/(a7-0.5*a3)) ** cf
        npstep = 1.+np.tanh((a7-0.5*a3-x)/1.e-6)
        cf = 0.5*(1.+np.tanh((a6-0.2)/1.e-2))*a6+0.1
        y = y + a4 * ((abs(1-yt)) **cf) * 0.5 * npstep
    ...
    def core_prof(self, x, a1=0, a2=0, a3=0, a4=0):
    ...
    def mtanh_prof(self, x, a1=0, a2=0, a3=0, a4=0, a5=0, a6=0, a7=0, a8=0): #mtanh_prof
        ...
        y = y + (a6 - y) * np.exp(-1.*(x/(np.abs(a7)+0.2))**np.abs(a8))
    ...
    def spline_fit(self,datx,daty,datw,knots_num,k=2):
        ...
        result = lmfit.minimize(self.spline_residual,p,args=(datx,daty,datw,knots_num,0.1,k),method='leastsq') # 0.1 min delx b/w knots
        p = result.params
    ...
    def draw_plot_part(self,fig,flag):
        ...
        if np.sum(self.__dict__['%s_prof'%flag]['fit_old']) > 0:
            ...
            try: 
                ...
                valerr = self.make_error_boundary(flag,self.param,False)
    ...
    def draw_plot(self,fig_ex=None,second=False):
        ...
        if not fig_ex == None:
            ...
            #fig.canvas.manager.set_window_title('Profile FIT')
    ...
    def make_error_boundary(self,flag,param,isnew=True):

        fit_type = self.fit_opt['func_type'][flag]
        self.post['errsample'][flag] = None;
        if isnew: 
            popt = self.post['popt'][flag]
            popte= self.post['popte'][flag]
            if flag in self.post['poptc'].keys(): poptc= self.post['poptc'][flag]
            else: poptc = None
        else:
        ...
        for i in range(popt.shape[0]):
            val = popt[i]
            inps['a%i'%(i+1)] = val
        errnom = func(self.fit_eq['psin2'],**inps)

        if poptc is None:
            self.post['errnom'][flag] = errnom
            self.post['errstd'][flag] = errnom*0.
            return errnom, errnom*0.

        valid_indices = []; valid_means = [];
        for i in range(popte.shape[0]):
            stderr = popte[i]
            if stderr>0 and not np.isnan(stderr): 
                valid_indices.append(i); 
                valid_means.append(popt[i]);

        if not (len(valid_means)==poptc.shape[0]): 
            print('>>> ',flag,'mean',valid_means,'err',popte,'cov',poptc)
            return errnom, errnom*0.0

        n_sample  = 3001
        sampled_params = multivariate_normal.rvs(mean=valid_means, cov=poptc, size=n_sample)
        y_samples = []

        inps      = {}
        for j in range(popt.shape[0]): inps['a%i'%(j + 1)] = popt[j]

        for params in sampled_params:
            
            for j, index in enumerate(valid_indices):
                param_value = params[j]
                param_name  = 'a%i'%(index + 1)
                inps[param_name] = param_value

            for j in range(popt.shape[0]):
    
                param_name  = 'a%i'%(j + 1)
                param_value = inps[param_name]

                min_val = param[flag]['min'][j]
                max_val = param[flag]['max'][j]

                min_val = min_val * (1-0.2*np.sign(min_val))
                max_val = max_val * (1+0.2*np.sign(max_val))

                if param_value < min_val:
                    inps[param_name] = min_val
                elif param_value > max_val:
                    inps[param_name] = max_val
                else:
                    inps[param_name] = param_value

            y_model = func(self.fit_eq['psin2'],**inps)
            if not np.any(np.isnan(y_model)): 
                y_samples.append(y_model)

        y_samples = np.array(y_samples)
        self.post['errsample'][flag] = {}

        if self.fit_opt['use_rho'][flag]:
            self.post['errsample'][flag]['rho'] = np.copy(self.fit_eq['psin2'])
            self.post['errsample'][flag]['psi'] = self.fit_eq['rho_to_psi'](self.fit_eq['psin2'])
        else:
            self.post['errsample'][flag]['rho'] = self.fit_eq['psi_to_rho'](self.fit_eq['psin2'])
            self.post['errsample'][flag]['psi'] = np.copy(self.fit_eq['psin2'])
    ...
    def make_psiRZ_extended(self,psi,Z,islfs=True):
        ...
        points = np.vstack((R, np.full_like(R, Z))).T

        psi_map = self.fit_eq['psif'](points)
        psi_map[0] = 0.0
    ...
    def mapping_variables(self):
        ...
        self.fit_eq['psif'] = RegularGridInterpolator((self.fit_eq['R'],self.fit_eq['Z']),(self.fit_eq['eq'].psirz-self.fit_eq['eq'].smag).T/(self.fit_eq['eq'].sbdy-self.fit_eq['eq'].smag))
        zmag_array = np.full_like(self.fit_eq['R'],self.device['inter1Z'])
        points = np.vstack((self.fit_eq['R'], zmag_array)).T

        psin  = self.fit_eq['psif'](points)

        rmag_array = np.full_like(self.fit_eq['Z'],self.device['inter2R'])
        points = np.vstack((rmag_array,self.fit_eq['Z'])).T
        psin2 = self.fit_eq['psif'](points)         
        ...
        z_array                 = np.full_like(self.device['intr1'],self.device['inter1Z'])
        points                  = np.vstack((self.device['intr1'],z_array)).T
        self.device['intp1']    = self.fit_eq['psif'](points)
        r_array                 = np.full_like(self.device['intz2'],self.device['inter2R'])
        points                  = np.vstack((r_array,self.device['intz2'])).T
        self.device['intp2']    = self.fit_eq['psif'](points)
...
#           Re = self.Rsep + 0.001
            ...
            z_array = np.full_like(R2,0.)
            points  = np.vstack((R2,z_array)).T
            self.device['tcip%i'%(j+1)] = self.fit_eq['psif'](points)
            self.device['tcir%i'%(j+1)] = np.copy(R1)
            
        z_array = np.full_like(self.fit_eq['R'],self.fit_eq['zmag'])
        points  = np.vstack((self.fit_eq['R'],z_array)).T
        psin = self.fit_eq['psif'](points)
        ...
        z_array = np.full_like(RO,self.fit_eq['zmag'])
        points  = np.vstack((RO,z_array)).T
        psin = self.fit_eq['psif'](points)
        psin[0] = 0.
        psin[-1] = 1.
    ...
    def initialise_variables(self):
        ...
        self.post['poptc']       = dict()
        self.post['errnom']      = dict()
        self.post['errsample']   = dict()
    ...
    def main_run(self):
        ...
        # Legacy interp2d check
        try: 
            self.fit_eq['psif'](np.array([[self.fit_eq['R'][0],self.fit_eq['Z'][0]]]))
        except:
            print(">>> Warning: Loaded object is a legacy interp2d. Recreating as RegularGridInterpolator.")
            self.fit_eq['psif'] = RegularGridInterpolator((self.fit_eq['R'],self.fit_eq['Z']),(self.fit_eq['eq'].psirz-self.fit_eq['eq'].smag).T/(self.fit_eq['eq'].sbdy-self.fit_eq['eq'].smag))
