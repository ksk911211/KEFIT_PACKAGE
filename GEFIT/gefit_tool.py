...
from exec_dirs import kindata_dir,mds_dir, mse_good_ch, nubeam_config
...
def make_nubeam_opt(sim,filename='nubeam_opt',eqtype=1):
    ...
    mfile = nubeam_config + '/%i/mfile.dat'%sim.year
    sfile = nubeam_config + '/%i/sfile.dat'%sim.year

    if not sim.StrVar119.get() == '':   f.write('MFILE = %s\n'%sim.StrVar119.get())
    else:                               f.write('MFILE = %s\n'%mfile)
    if not sim.StrVar120.get() == '':   f.write('SFILE = %s\n'%sim.StrVar120.get())
    else:                               f.write('SFILE = %s\n'%sfile)
...
def load_eq_data(sim,filename,filename2=None,skip=False):
    ...
    if not skip:
        ...
        if not os.path.isfile(f_file):  
            ...
            brf = RegularGridInterpolator((eq.r,eq.z),eq.br.T)
            bzf = RegularGridInterpolator((eq.r,eq.z),eq.bz.T)
            btf = RegularGridInterpolator((eq.r,eq.z),eq.bt.T)      
...
def efit_post_process(sim,index,gfile_dir,w_dir,mse_dir,pres_dir,j_dir,map_dir,skip=False):
    ...
    brf = RegularGridInterpolator((eq2.r,eq2.z),eq2.br.T)
    bzf = RegularGridInterpolator((eq2.r,eq2.z),eq2.bz.T)
    btf = RegularGridInterpolator((eq2.r,eq2.z),eq2.bt.T)       
