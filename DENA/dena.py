...
class kstar_density_tool:
        ...
        def _make_input_frame(self):
                ...
                rowv = rowv + 1
                self.l1 = tk.Label(self.home['page1'], text="================== DRIFT CORRECTION ===================",justify='center')         
                self.l1.grid(row=rowv,column=0,columnspan=10,pady=5)                    

                rowv = rowv + 1
                self.note_in['frame12'] = tk.Frame(self.home['page1'])
                self.note_in['frame12'].grid(row=rowv,column=0,columnspan=10)                                   
                self.l1 = tk.Label(self.note_in['frame12'], text='DRIFT1')
                self.l1.pack(side='left')
                self.note_in['e31'] = tk.Entry(self.note_in['frame12'],textvariable=self.note_in['DRIFT1'],width=5,justify='center')
                self.note_in['e31'].pack(side='left')

                b1 = tk.Button(self.note_in['frame12'],  text="UPDATE", width = 6,command=lambda: self._update_drift())
                b1.pack(side='left',padx=3)                                     
                if self.read_mode: b1.config(state='disabled')
        ...
        def _draw_overview(self):
                ...
                for ch in range(1,self.tci['nch']+1):
                        drift_corr = float(self.note_in['DRIFT1'].get()) / self.fit.device['tci_L']['tci%02i'%(ch)]
                        if self.tci[ch]['is']: 
                                ...
                                if drift_corr==0:
                                        line1, = self.figure['axes']['home'][count].plot(self.tci[ch]['val'][0],self.tci[ch]['val'][1]-drift_corr*self.tci[ch]['val'][0])
                                else:
                                        line1, = self.figure['axes']['home'][count].plot(self.tci[ch]['val'][0],self.tci[ch]['val'][1]-drift_corr*self.tci[ch]['val'][0],color='b')
...
#               if self.out_rt1 == None: self.note_in['r5'].configure(state='disabled')
        ...
        def _update_drift(self):
                ...
                self.figure['name']['home'].canvas.draw_idle()
                lenp = len(self.tcirawplot)
                if lenp > 0:
                        for i in range(lenp):
                                plot = self.tcirawplot[i]; plot.remove(); 
                                self.figure['pegend'][i+1].remove(plot); self.figure['legend'][i+1].remove('TCI%02i [$10^{19}/m^3$]'%(i+1));
                ...
                for ch in range(1,self.tci['nch']+1):
                        ...
                        if self.tci[ch]['is']: 
                                ...
                                self.tci[ch]['val'][0] = self.tci[ch]['val'][0][0:lenm]; self.tci[ch]['val'][1] = self.tci[ch]['val'][1][0:lenm]
                                line1, = self.figure['axes']['home'][count].plot(self.tci[ch]['val'][0],self.tci[ch]['val'][1]-drift_corr*self.tci[ch]['val'][0],color='b')
                                self.figure['pegend'][count].append(line1); self.figure['legend'][count].append('TCI%02i [$10^{19}/m^3$]'%ch);
                                self.figure['axes']['home'][count].legend(self.figure['pegend'][count],self.figure['legend'][count],loc='upper right')
                ...
                for k in range(len(self.profiles['times'])):
                        ...
                        for i in range(tot_nch):
                                ...
                                if not dat == None:
                                        ...
                                        if i in range(2,7):
                                                drift_corr = float(self.note_in['DRIFT1'].get()) * time1 / self.fit.device['tci_L']['tci%02i'%(i-1)] * 1.e-3
                                        else:
                                                drift_corr = 0.;
                                        self.profiles['infile']['tciv'][time1][i] = tciavg - drift_corr
...
#                       self.fit.param['ne']['val'][1] = min_ped
        ...
        def _initialise_variables(self):
                ...
                self.note_in['DRIFT1'] = tk.StringVar()
