...
class eped_ftool:
    ...
    def modify_profile(self):
        ...
        z_array = np.full_like(R1,te_datz)
        te_psi = self.psif(np.vstack((R1,z_array)).T)
        z_array = np.full_like(R2,ne_datz)
        ne_psi = self.psif(np.vstack((R2,z_array)).T)
        z_array = np.full_like(R3,ti_datz)
        ti_psi = self.psif(np.vstack((R3,z_array)).T)
    ...
    def make_psiRZ_extended(self,psi,Z):
        ...
        z_array = np.full_like(R,Z)
        psi_map = self.psif(np.vstack((R,z_array)).T))
        psi_map[0] = 0.0
    ...
    def mapping_variables(self):
        ...
        if not (self.use_chease):
            ...
            self.psif = RegularGridInterpolator((self.R,self.Z),(self.eq.psirz-self.eq.smag).T/(self.eq.sbdy-self.eq.smag))

            z_array = np.full_like(self.R,0.0)
            points  = np.vstack((self.R,z_array)).T

            psin = self.psif(points)
            ...
            self.psif = RegularGridInterpolator((ch.eq.R,ch.eq.Z),(ch.eq.psirz-ch.eq.smag).T/(ch.eq.sbdy-ch.eq.smag))
